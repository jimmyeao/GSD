...
function generateTerrain()
    gameState.terrain = {}
    local segmentWidth = gameState.levelWidth / Config.TERRAIN_SEGMENTS
    
    for i = 0, Config.TERRAIN_SEGMENTS do
        local x = i * segmentWidth
        -- Base height + random variation
        local y = Config.GROUND_HEIGHT + math.random(-50, 50)
        table.insert(gameState.terrain, x)
        table.insert(gameState.terrain, y)
    end
end
...
function generateStars()
    ...
    for i = 1, Config.STAR_COUNT do
        table.insert(gameState.stars, {
            x = math.random(0, gameState.levelWidth),
            y = math.random(0, Config.GROUND_HEIGHT),
...
function love.update(dt)
    if not gameState.player.alive then 
        -- Still update enemies rotation even if dead so they keep spinning
        updateEnemiesRotation(dt)
        return 
    end
...
function updateEnemiesRotation(dt)
    for _, e in ipairs(gameState.enemies) do
        e.angle = (e.angle or 0) + dt * 2
    end
end
...
function createExplosion(x, y, count, color)
    for i = 1, count do
        ...
        local speed = math.random() * 150 + 50
        table.insert(gameState.particles, {
            x = x,
            y = y,
...
function love.draw()
    ...
    love.graphics.rectangle("fill", 0, 0, gameState.levelWidth, gameState.height)
    
    -- Draw Stars
    love.graphics.setColor(1, 1, 1)
    for _, star in ipairs(gameState.stars) do
        -- Parallax effect: stars move slower than camera
        local screenX = star.x - gameState.camera.x * 0.5
        if screenX > -10 and screenX < gameState.width + 10 then
            love.graphics.setColor(star.brightness, star.brightness, star.brightness)
            love.graphics.circle("fill", screenX, star.y, star.size)
        end
    end
    
    -- Draw Terrain
    if #gameState.terrain > 0 then
        -- Fill Terrain
        love.graphics.setColor(0.3, 0.3, 0.35)
        love.graphics.polygon("fill", unpack(gameState.terrain))
        
        -- Draw terrain top border
        love.graphics.setColor(0.6, 0.6, 0.6)
        love.graphics.setLineWidth(2)
        love.graphics.polygon("line", unpack(gameState.terrain))
        love.graphics.setLineWidth(1)
        
        -- Fill below terrain to create ground
        local groundPoints = {}
        table.insert(groundPoints, 0)
        table.insert(groundPoints, gameState.height)
        table.insert(groundPoints, gameState.levelWidth)
        table.insert(groundPoints, gameState.height)
        
        -- Add terrain points in reverse to close the loop properly
        for i = #gameState.terrain, 1, -2 do
            table.insert(groundPoints, gameState.terrain[i])
            table.insert(groundPoints, gameState.terrain[i-1])
        end
        
        love.graphics.setColor(0.1, 0.1, 0.1)
        love.graphics.polygon("fill", unpack(groundPoints))
    end
    ...
    if gameState.player.alive then
        ...
        love.graphics.push()
        love.graphics.translate(gameState.player.x, gameState.player.y)
        love.graphics.rotate(gameState.player.angle)
        ...
        if love.keyboard.isDown("left") or love.keyboard.isDown("right") then
            ...
            love.graphics.circle("fill", -18, 0, 4 + math.random(2))
        end
    ...
    for _, p in ipairs(gameState.particles) do
        if p.type == "fuel" then
            ...
            -- Fade out particles
            local alpha = math.min(1, p.life / Config.EXPLOSION_LIFE)
            love.graphics.setColor(p.color[1], p.color[2], p.color[3], alpha)
            love.graphics.circle("fill", p.x, p.y, p.size or 3)
        end
    end
