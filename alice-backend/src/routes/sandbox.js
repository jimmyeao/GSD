...
router.post('/', async (req, res) => {
  try {
    ...
    const containerName = `alice-${req.user.id}-${projectId}-${Date.now()}`;
...
router.post('/build', async (req, res) => {
  try {
    ...
    const workspacePath = join(WORKSPACES_ROOT, String(req.user.id), String(projectId));
    const imageTag = tag || `alice-${req.user.id}-${projectId}:latest`;
...
router.post('/compose/up', async (req, res) => {
  try {
    ...
    const projectName = `alice-${req.user.id}-${projectId}`;
    const envVars = JSON.parse(project.env_vars || '{}');
