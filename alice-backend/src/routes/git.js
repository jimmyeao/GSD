...
router.post('/:projectId/init', async (req, res) => {
  ...
  try {
    ...
    try { await git.raw(['config', 'user.name', req.user.username || 'Alice User']); } catch { /* non-fatal */ }
    try { await git.raw(['config', 'user.email', req.user.email || 'user@alice.local']); } catch { /* non-fatal */ }
