 ...
 *
 * Image: `alice-love-runner:latest`, built from ./docker/love-runner.
...
export const LOVE_IMAGE_TAG = 'alice-love-runner:latest';
...
async function ensureNetwork() {
  try { await docker.getNetwork(ALICE_NETWORK).inspect(); }
  catch { await docker.createNetwork({ Name: ALICE_NETWORK, Driver: 'bridge' }); }
}
...
export async function startLoveContainer(workspacePath, name) {
  ...
  // pull` (there's no remote registry for alice-love-runner).
  ...
  const container = await docker.createContainer({
    ...
    Labels: { 'alice.managed': 'true', 'alice.love': 'true' },
    ...
    HostConfig: {
      ...
      NanoCpus: 2 * 1e9,
      AutoRemove: false,
      // Attach to the shared Alice network so the preview proxy can find the
