...
/** Ensure the shared GSD bridge network exists. */
async function ensureNetwork() {
  try {
    await docker.getNetwork(GSD_NETWORK).inspect();
  } catch {
    await docker.createNetwork({ Name: GSD_NETWORK, Driver: 'bridge' });
  }
}
 ...
 * @param {string} tag - image tag (e.g. 'gsd-user-2-proj-1:latest')
...
export async function buildImage(contextPath, tag, onProgress = () => {}) {
  const stream = await docker.buildImage(
    ...
    { t: tag, nocache: false, labels: { 'gsd.managed': 'true' } }
  );
  ...
  return new Promise((resolve, reject) => {
    docker.modem.followProgress(stream, (err, output) => {
      ...
      if (event.error) onProgress({ error: event.error });
    });
  });
}

/**
 * List GSD-managed Docker images.
 * @returns {Promise<Array<{id, tags, size, created}>>}
 */
export async function listImages() {
  const images = await docker.listImages({ filters: { label: ['gsd.managed=true'] } });
