...
router.post('/:projectId/love/run', async (req, res) => {
  ...
  try {
    ...
    if (!(await loveImageExists())) {
      console.log('[love] building gsd-love-runner image (first run, ~60s expected)');
    ...
    const ins = stmts.insertContainer.run(req.params.projectId, req.user.id, 'gsd-love-runner:latest');
    ...
    res.json({
      ...
      // IMPORTANT: the `path` query param is the path noVNC uses for its
      // WebSocket connection. It MUST match the NGINX location block that
      // forwards Upgrade headers — in this deployment, that's /api/*. If
      // the path doesn't include /api/ the browser's WS request goes to a
      // location NGINX doesn't proxy and fails with "failed to connect".
      url: `/api/preview/${containerId}/vnc.html?autoconnect=1&resize=scale&reconnect=1&path=api/preview/${containerId}/websockify`,
    });
  } catch (err) {
