...
/** Given a container DB row, return { ip, port } to proxy to. */
async function resolveTarget(row, queryPort) {
  ...
  // Default port by image: love-runner → 6080 (noVNC), else 3000.
  let port;
  if (queryPort) port = parseInt(queryPort, 10);
  else if (row.image === 'gsd-love-runner:latest') port = 6080;
  else port = 3000;
  return { ip: info.ip, port };
}
...
router.all('/:containerId/*', async (req, res) => {
  try {
    ...
    const { ip, port } = await resolveTarget(row, req.query.port);
...
export function handleUpgrade(req, socket, head, authFn) {
  (async () => {
    try {
      ...
      const { ip, port } = await resolveTarget(row, null);
