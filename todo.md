 ---
  ...
  Not code-server (too heavy, separate auth, one process per user). Instead, embed Monaco Editor directly as a JS library in the existing GSD frontend — no new
  ports, no iframes, inherits your JWT auth automatically.

  What you'd get

  - Code view in GSD — tab between Chat and Code modes
  ...
  - Pre-built Docker images: gsd-node:20, gsd-python:3.12 — fast startup
